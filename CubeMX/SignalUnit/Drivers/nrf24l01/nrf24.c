...
static void nRF24_ReadMBReg(NRF24 *nrf24, uint8_t reg, uint8_t *pBuf, uint8_t count)
{
	nRF24_CSN_L(nrf24);
	nRF24_LL_RW(nrf24, reg);
	while (count--)
...
static void nRF24_WriteMBReg(NRF24 *nrf24, uint8_t reg, uint8_t *pBuf, uint8_t count)
{
	nRF24_CSN_L(nrf24);
	nRF24_LL_RW(nrf24, reg);
	while (count--)
...
void nRF24_SetPayloadWithAck(NRF24 *nrf24, uint8_t mode)
{
	...
	if (mode)
	{
		nRF24_WriteReg(nrf24, nRF24_REG_FEATURE, reg | nRF24_FEATURE_EN_ACK_PAY);
	} else
	{
		nRF24_WriteReg(nrf24, nRF24_REG_FEATURE, reg & ~ nRF24_FEATURE_EN_ACK_PAY);
	}
...
void nRF24_SetAutoRetr(NRF24 *nrf24, uint8_t ard, uint8_t arc)
{
	...
	nRF24_WriteReg(nrf24, nRF24_REG_SETUP_RETR, (uint8_t) ((ard << 4) | (arc & nRF24_MASK_RETR_ARC )));
}
...
void nRF24_SetRXPipe(NRF24 *nrf24, uint8_t pipe, uint8_t aa_state, uint8_t payload_len)
{
	uint8_t reg;

	// Enable the specified pipe (EN_RXADDR register)
	reg = (nRF24_ReadReg(nrf24, nRF24_REG_EN_RXADDR) | (1 << pipe)) & nRF24_MASK_EN_RX;
	nRF24_WriteReg(nrf24, nRF24_REG_EN_RXADDR, reg);

	// Set RX payload length (RX_PW_Px register)
	nRF24_WriteReg(nrf24, nRF24_RX_PW_PIPE[pipe], payload_len & nRF24_MASK_RX_PW);
...
{
	return ((nRF24_ReadReg(nrf24, nRF24_REG_FIFO_STATUS) & nRF24_MASK_TXFIFO ) >> 4);
}
...
static nRF24_RXResult nRF24_ReadPayloadGeneric(NRF24 *nrf24, uint8_t *pBuf, uint8_t *length, uint8_t dpl)
{
	uint8_t pipe;
